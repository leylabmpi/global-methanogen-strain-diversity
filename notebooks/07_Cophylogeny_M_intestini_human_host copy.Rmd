---
title: "07_Cophylogeny_M_intestini_human_host"
...
bac_tree_path <- "data/M_intestini_bestTree.faa_refined.tre"
host_tree_path <- "data/Host_tree_best.tree"
assoc_path <- "data/HostTip_BacTip_MI_assoc_file.txt"
...
assoc_df
# check tip labels (remove extra spaces or trailing characters)
...
```{r}
run_paco <- function(bac_tree, host_tree, assoc_df, n_perm = 999, output_dir = "./PACO_out_MI/") {
...
# Plot PACo Residuals by sampling locations in Gabon only

```{r}
library(ggplot2)

# Assign HostLocation
assoc_df$HostLocation <- dplyr::case_when(
  grepl("^G0[0-2][0-9]{2}", assoc_df$Host) ~ "Lambaréné",
  grepl("^G0[3-9][0-9]{2}", assoc_df$Host) ~ "Ikobey",
  TRUE ~ NA_character_
)

assoc_df$HostLocation <- factor(
  assoc_df$HostLocation,
  levels = c("Lambaréné", "Ikobey")
)
...
# Compute dynamic y-position for label
y_max <- max(assoc_df$residuals, na.rm = TRUE)
y_label <- y_max * 1.05   # 5% above max value

# Main plot
p <- ggplot(assoc_df, aes(x = HostLocation, y = residuals)) +
  geom_boxplot(
    fill = NA,
    color = "black",
    outlier.shape = NA
  ...
  # Global test (sufficient for 2 groups)
  stat_compare_means(
    method = "kruskal.test",
    label = "p.format",
    label.y = y_label,
...
p
# Save
ggsave("M_intestini_cophylogeny_PACo_residuals.pdf", p, width = 60, height = 50, units = "mm", dpi = 300)
ggsave("M_intestini_cophylogeny_PACo_residuals.svg", p, width = 60, height = 50, units = "mm", dpi = 300, device = svglite::svglite)
